...
LSTM::Model::Model(LSTM::Shape const shape)
  ...
  , output(shape.output_size * (shape.num_cells * shape.num_layers + 1)) // 256 * (200 * 2 + 1) = 256 * 401 = 102,656 (includes 256 biases)
{
  ...
  for (size_t i = 0; i < shape.num_layers; i++) { // 2 iterations
    size_t weight_size = 1 + shape.num_cells * (i > 0 ? 2 : 1) + shape.output_size; // Layer 0: 1 + 200*1 + 256 = 457, Layer 1: 1 + 200*2 + 256 = 657 (includes 1 bias)
    total_weight_size += gates_count * shape.num_cells * weight_size; // Layer 0: 3*200*457=274,200, Layer 1: 3*200*657=394,200
  }
  weights.resize(total_weight_size);              // 274,200 + 394,200 = 668,400
}
...
void LSTM::Model::LoadFromDisk(const char* const dictionary, int32_t bits, int32_t exp) {
  ...
  size_t hidden_size = shape.num_cells * shape.num_layers + 1; // 200 * 2 + 1 = 401
  ...
  if ((bits > 0) && (bits <= 16)) {
    ...
    for (size_t i = 0; i < shape.output_size; i++) { // 256 iterations
      for (size_t j = 0; j < hidden_size; j++)       // 401 iterations
        output[i * hidden_size + j] = Posit<16, 1>::Decode(file.getBits(bits)) * scale; // output[i * 401 + j]
    }

    // Load weights
    size_t weight_offset = 0;
    for (size_t layer = 0; layer < shape.num_layers; layer++) { // 2 iterations
      size_t weight_size = 1 + shape.num_cells * (layer > 0 ? 2 : 1) + shape.output_size; // Layer 0: 457, Layer 1: 657 (includes 1 bias)
      for (size_t gate = 0; gate < 3; gate++) {     // 3 iterations (forget, input, output gates)
        for (size_t cell = 0; cell < shape.num_cells; cell++) { // 200 iterations
          for (size_t idx = 0; idx < weight_size; idx++) { // Layer 0: 457 iterations, Layer 1: 657 iterations
      ...
      weight_offset += 3 * shape.num_cells * weight_size; // Layer 0: += 274,200, Layer 1: += 394,200
    }
  }
  else {
    float v;
    ...
    for (size_t i = 0; i < shape.output_size; i++) { // 256 iterations
      for (size_t j = 0; j < hidden_size; j++) {     // 401 iterations
        ...
        output[i * hidden_size + j] = v;             // output[i * 401 + j]
      }
    }
    ...
    for (size_t layer = 0; layer < shape.num_layers; layer++) { // 2 iterations
      ...
      for (size_t gate = 0; gate < 3; gate++) {     // 3 iterations
        for (size_t cell = 0; cell < shape.num_cells; cell++) { // 200 iterations
          for (size_t idx = 0; idx < weight_size; idx++) { // Layer 0: 457, Layer 1: 657
            if (file.blockRead(reinterpret_cast<uint8_t*>(&v), sizeof(float)) != sizeof(float)) break;
            size_t index = weight_offset + gate * shape.num_cells * weight_size + cell * weight_size + idx;
            weights[index] = v;
          }
...
void LSTM::Model::SaveToDisk(const char* const dictionary, int32_t bits, int32_t exp) {
  ...
  if ((bits > 0) && (bits <= 16)) {
    ...
    for (size_t i = 0; i < shape.output_size; i++) { // 256 iterations
      for (size_t j = 0; j < hidden_size; j++) {     // 401 iterations
        if ((w = std::fabs(output[i * hidden_size + j])) > max_w) // output[i * 401 + j]
          max_w = w;
      }
    }
    ...
    for (size_t layer = 0; layer < shape.num_layers; layer++) { // 2 iterations
      size_t weight_size = 1 + shape.num_cells * (layer > 0 ? 2 : 1) + shape.output_size; // Layer 0: 457, Layer 1: 657 (includes 1 bias)
      size_t layer_total = 3 * shape.num_cells * weight_size; // Layer 0: 274,200, Layer 1: 394,200
    ...
    for (size_t i = 0; i < shape.output_size; i++) { // 256 iterations
      for (size_t j = 0; j < hidden_size; j++)       // 401 iterations
        file.putBits(Posit<16, 1>::Encode(output[i * hidden_size + j] / scale), bits); // output[i * 401 + j]
    }
    ...
    for (size_t i = 0; i < shape.output_size; i++) { // 256 iterations
      for (size_t j = 0; j < hidden_size; j++) {     // 401 iterations
        v = output[i * hidden_size + j];             // output[i * 401 + j]
...
Lstm::Lstm(
  ...
  : simd(simdType)
  , layer_input(shape.horizon * shape.num_layers * (1 + shape.num_cells * 2)) // 100 * 2 * (1 + 200*2) = 100 * 2 * 401 (includes biases)
  , output_layer(shape.horizon * shape.output_size * (shape.num_cells * shape.num_layers + 1)) // 100 * 256 * (200*2 + 1) = 100 * 256 * 401 (includes biases)
  , output(shape.horizon * shape.output_size)         // 100 * 256 = 25,600
  , logits(shape.horizon * shape.output_size)         // 100 * 256 = 25,600
  , hidden(shape.num_cells * shape.num_layers + 1)    // 200 * 2 + 1 = 401 (includes 1 bias)
  ...
  , epoch(0)
{
  hidden[hidden.size() - 1] = 1.f; // bias (hidden[400] = 1.f, since hidden.size() = 401)

  size_t max_layer_size = 1 + num_cells * 2;         // 1 + 200*2 = 401 (includes 1 bias)
  ...
  for (size_t e = 0; e < horizon; e++) {             // 100 iterations
    for (size_t layer = 0; layer < num_layers; layer++) { // 2 iterations
      size_t layer_input_size = 1 + num_cells * (layer > 0 ? 2 : 1); // Layer 0: 1+200*1=201, Layer 1: 1+200*2=401 (includes 1 bias)
      layer_input[e * num_layers * max_layer_size + layer * max_layer_size + (layer_input_size - 1)] = 1.f; // bias
    }
  }
  ...
  for (size_t i = 0; i < num_layers; i++) {          // 2 iterations
    size_t layer_input_size = 1 + num_cells * (i > 0 ? 2 : 1); // Layer 0: 201, Layer 1: 401 (includes 1 bias)
    layers.push_back(
      std::make_unique<LstmLayer>(
        simdType,
        layer_input_size + output_size,              // Layer 0: 201+256=457, Layer 1: 401+256=657
        output_size,                                 // 256
        num_cells,                                   // 200
        horizon                                      // 100
      )
...
void Lstm::SoftMaxSimdAVX2() {
  size_t const len = hidden.size();                  // 401
  size_t const hidden_size = num_cells * num_layers + 1; // 200 * 2 + 1 = 401 (includes 1 bias)
  ...
  for (size_t i = 0; i < output_size; i++) {         // 256 iterations
    ...
    logits[epoch * output_size + i] = dot256_ps_avx2( // logits[epoch * 256 + i]
      &hidden[0],
      &output_layer[epoch * output_size * hidden_size + i * hidden_size], // &output_layer[epoch * 256 * 401 + i * 401]
      len,                                           // 401
      0.f
    );
  }
...
float* Lstm::Predict(uint8_t const input) {
  size_t const max_layer_size = 1 + num_cells * 2;  // 1 + 200*2 = 401 (includes 1 bias)

  // Create temporary array to pass a slice of layer_input to ForwardPass
  Array<float, 32> temp_input(1 + num_cells * 2 + output_size); // 1 + 200*2 + 256 = 657

  for (size_t i = 0; i < layers.size(); i++) {      // 2 iterations
    // Copy from hidden to layer_input
    size_t layer_input_size = 1 + num_cells * (i > 0 ? 2 : 1); // Layer 0: 201, Layer 1: 401 (includes 1 bias)
    for (size_t j = 0; j < num_cells && j < layer_input_size - 1; j++) { // 200 iterations
      layer_input[epoch * num_layers * max_layer_size + i * max_layer_size + j] = hidden[i * num_cells + j];
    }

    // Prepare temp_input for this layer
    for (size_t j = 0; j < layer_input_size; j++) { // Layer 0: 201, Layer 1: 401
...
void Lstm::Perceive(const uint8_t input) {
  ...
  if (epoch == 0) {
    for (int epoch_ = static_cast<int>(horizon) - 1; epoch_ >= 0; epoch_--) { // starts at 99, goes to 0
      for (int layer = static_cast<int>(layers.size()) - 1; layer >= 0; layer--) { // starts at 1, goes to 0
        ...
        // Prepare temp_input
        size_t layer_input_size = 1 + num_cells * (layer > 0 ? 2 : 1); // Layer 0: 201, Layer 1: 401 (includes 1 bias)
        for (size_t j = 0; j < layer_input_size; j++) { // Layer 0: 201, Layer 1: 401
  ...
  for (size_t i = 0; i < output_size; i++) {         // 256 iterations
    ...
    for (size_t j = 0; j < hidden.size(); j++) {     // 401 iterations
...
void Lstm::Reset() {
  ...
  memset(
    &output_layer[0],
    0,
    horizon * output_size * hidden_size * sizeof(float)); // 100 * 256 * 401 * 4 = 41,062,400 bytes
  memset(
    &hidden[0],
    0,
    (hidden.size() - 1) * sizeof(float));          // (401 - 1) * 4 = 1,600 bytes
  hidden[hidden.size() - 1] = 1.f;                   // hidden[400] = 1.f (bias)
  ...
  for (size_t e = 0; e < horizon; e++) {            // 100 iterations
    ...
    for (size_t layer = 0; layer < num_layers; layer++) { // 2 iterations
      size_t layer_input_size = 1 + num_cells * (layer > 0 ? 2 : 1); // Layer 0: 201, Layer 1: 401 (includes 1 bias)
      for (size_t k = 0; k < layer_input_size - 1; k++) { // Layer 0: 200, Layer 1: 400
...
void Lstm::LoadModel(LSTM::Model& model) {
  ...
  for (size_t layer = 0; layer < layers.size(); layer++) { // 2 iterations
    ...
    size_t weight_size = 1 + num_cells * (layer > 0 ? 2 : 1) + output_size; // Layer 0: 457, Layer 1: 657
    ...
    weight_offset += 3 * num_cells * weight_size;    // Layer 0: += 274,200, Layer 1: += 394,200
  }
}
