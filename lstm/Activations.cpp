...
#endif
void Tanh::RunSimdAVX2(float* f, size_t const len) const {
    ...
    for (size_t i = 0; i < limit; i += SIMDW) {
        ...
        __m256 v3 = _mm256_castsi256_ps(
            _mm256_add_epi32(
                _mm256_castps_si256(_mm256_add_ps(v2, v1)),
        ...
        _mm256_storeu_ps(
            f + i,
            _mm256_div_ps(_mm256_add_ps(v3, v4), _mm256_sub_ps(v3, v4))
        );
    }
...
__attribute__((target("avx2,fma")))
#endif
void Logistic::RunSimdAVX2(float* f, size_t const len) const {
    static constexpr size_t SIMDW = 8;
    ...
    for (size_t i = 0; i < limit; i += SIMDW) {
        ...
        _mm256_storeu_ps(
            ...
            _mm256_add_ps(
                _mm256_div_ps(
                    _mm256_mul_ps(_mm256_add_ps(v3, v4), vec_half),
                    _mm256_sub_ps(v3, v4)
                ),
                vec_half
            )
