...
Image24BitModel::Image24BitModel(Shared* const sh, const uint64_t size) : 
  ...
  map{ /* StationaryMap : BitsOfContext, InputBits, Scale=64, Rate=16  */
    ...
    /*nSM1:72-80*/ {sh,11,1,74}, {sh,11,1,74}, {sh,11,1,74}, {sh,11,1,74}, {sh,11,1,74}, {sh,11,1,74}, {sh,11,1,74}, {sh,11,1,74}, {sh,11,1,74},
    /*nSM1:81-82*/ {sh,11,1,74}, {sh,11,1,74},
...
void Image24BitModel::update() {
  ...
  if( bpos == 0 ) {
    ...
    if( x > 0 || (isPNG == 0u)) {
      ...
      mapContexts[++j] = NW;
      mapContexts[++j] = 0;
      mapContexts[++j] = (W * 2 + N * 3 + NE - NW) / 5;
      mapContexts[++j] = W;
      mapContexts[++j] = (NW + N * 2) / 3;
      mapContexts[++j] = (W * 4 - WW) / 3;
